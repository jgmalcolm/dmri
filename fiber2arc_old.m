function [xx ss vv] = fiber2arc_old(ff, m, eig, T, origin)
  ...
  % transform
  u = T*[u-origin 1]' + origin';
  v = T(1:3,1:3)*v'; % only rotate/scale
  ...
  xx = map(@(x) transform(x, T, origin'), xx);
  ...
  function ds = f2arc(x)
    ...
    if is_fig
      ds = [dsL 0 dsR] - off;  % keep all positive
    else
      ds = [-dsL 0 dsR] - off;
...
function X_ = transform(X, T, o)
  n = size(X,2);
  X_ = T*[center(X, o); ones(1,n)];
  X_ = center(X_, -o);
end
