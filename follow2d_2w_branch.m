function [pri sec] = follow2d_2w_branch(S, u, param)
  paths;
  ...
  points = [];
  for s = sec_
    [x X P] = deal(s(1:2), s(3:10), s(11:end));
    points(end+1).pos = x;
    points(end  ).X   = X;
    points(end  ).m   = unpack_watson(X);
    points(end  ).P   = reshape(P, [8 8]);
  end
