...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    uint n = mxGetNumberOfElements(prhs[0]);
    if ((nlhs == 5 && n != 11) || (nlhs == 8 && n != 17))
        mexErrMsgTxt("wrong size of state");
    ...
    if (nlhs == 5) {
        unpack(o+ 6, plhs+3, plhs+4, NULL,   m);
    } else {
        unpack(o+ 6, plhs+3, plhs+4, plhs+5, m);
        unpack(o+12, plhs+6, plhs+7, NULL,   m);
    }
}
